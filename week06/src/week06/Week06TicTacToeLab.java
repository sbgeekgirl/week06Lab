...
public class Week06TicTacToeLab {
	...
	public static void main(String[] args) {
        ...
        for(int x = 0; x < 3; x++){
            for (int y = 0; y < 3; y++){
                gameBoard[x][y] = Integer.toString(square);
                square ++;
            }
        }
        ...
        while(!winner){
            ...
            System.out.println("Enter you move, Player " + player);

            int move = in.nextInt();
            if (!(move < 1) && !(move > 9)){
                gameBoard = gamePlay(player, gameBoard, move);
            }else {
            ...
            drawBoard(gameBoard);
            if (round > 4){
                winner = checkForWinner(gameBoard, player);
            if(winner){
    ...
    public static String[][] gamePlay(String p, String[][] board,int move){
        switch(move){
            case 1:
                board[0][0] = p;
                break;
